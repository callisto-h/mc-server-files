...
└──────┬──────┘
       ...
       ▼
┌─────────────┐
│  Squaremap  │  live: proxies to Paper's built-in web server
│  :8080      │  offline: serves static map files from disk
└─────────────┘
```

**Velocity** is the main user-facing process. It handles player connections on port 25565 and uses the vane-velocity plugin to start Paper on demand when a player connects, and shut it down after 15 minutes empty. The server isn't directly exposed.
...
**Controller** is a small Flask app that has access to the Docker socket. It is the only container that can start or stop Paper. It also handles player whitelisting — looking up UUIDs from the Mojang API, running `whitelist add` on Paper via docker exec, and granting LuckPerms permissions on Velocity via docker exec. It exposes port 5000 to the host only. It also watches for Paper start/stop events via the Docker events API and automatically switches the squaremap nginx config between live and static mode.

**Squaremap** serves the live world map on port 8080 via nginx. When Paper is running, nginx proxies requests directly to the squaremap plugin’s built-in web server for live player positions and real-time tile updates. When Paper is offline, nginx automatically falls back to serving the last known static map files from disk.
...
│                            # NOT in repo — create with: cp squaremap/static.conf squaremap/default.conf
├── velocity_proxy/
│   ├── Dockerfile
...
# Create the initial squaremap nginx config
...
1. Zips the three world directories to a temp file on the SSD
1. Copies the zip to local storage (HDD), keeping the last 10
1. Uploads the zip to Google Drive (`remote:minecraft-backups`), keeping the last 3
...
|-----------------------|------|-----------------------------------------------------------------|
|`/health`              |GET   |Shows running status of Paper, Velocity, and Squaremap containers|
...
### 6. Set up the squaremap nginx config
...
The controller will automatically switch this to `live.conf` when Paper starts and back to `static.conf` when it stops. You do not need to manage this manually after initial setup.
